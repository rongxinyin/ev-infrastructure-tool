...
import datetime
import pandas as pd
from utilities.helpers import initialize_vehicles, add_home_charging
...
if __name__ == "__main__":
    ...
    for parking_lot in ['bldg-90']:
        ...
        # driving_pattern_input, start_time, run_period=30, 
        #                     l2_port_low=0, l2_port_high=0, l3_port_low=0, l3_port_high=0, 
        #                     l2_max_rate=19.2, l3_max_rate=60.0, adoption_rate=0.1
        ...
        l2_port_low = float(sys.argv[4])
        l2_port_high = float(sys.argv[5])
        l3_port_low = float(sys.argv[6])
        l3_port_high = float(sys.argv[7])
        l2_max_rate = float(sys.argv[8])
        l3_max_rate = float(sys.argv[9])
        adoption_rate = float(sys.argv[10])


        fleet_driving_patterns = json.loads(input_data)

        # add home charging information to the json data
        # TODO: find out if this is needed?
        add_home_charging(fleet_driving_patterns)

        driving_pattern_data = []
        for row in fleet_driving_patterns:
            if row['parking_lot'] == parking_lot:
                driving_pattern_data.append(create_driving_pattern(row, scenario='normal'))

        results = fleet_charging_management(parking_lot, driving_pattern_data, start_time=start_time, run_period=run_period, l2_port_low=l2_port_low, l2_port_high=l2_port_high, l3_port_low=l3_port_low, l3_port_high=l3_port_high, l2_max_rate=l2_max_rate, l3_max_rate=l3_max_rate, adoption_rate=adoption_rate)
        results.to_csv(os.path.join(site_path, f'fleet_vehicle_status_{adoption_rate}.csv'), index=False)
